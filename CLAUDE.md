...
- **`Pricing`** holds per-model USD/MTok rates plus cache multipliers (read 0.1×, 5m write 1.25×, 1h write 2×). A model's value is normally a flat `{ input:, output: }` hash; for models whose list price changes on a known date (e.g. `sonnet-5`'s introductory $2/$10 through 2026-08-31, then $3/$15) it's instead an array of tiers ordered oldest-first, each with an inclusive `until:` cutoff and the final tier open-ended. `cost_for`/`rates_for` take the cell's day and pick the active tier; a nil/`unknown` day falls back to the current open-ended tier. `normalize_model` reduces Claude ids to a concise name — dropping the `claude-` prefix and any trailing date stamp (`claude-haiku-4-5-20251001` → `haiku-4-5`) — which is also the id the report groups and displays by; non-Claude ids pass through untouched and cost `nil` (e.g. `<synthetic>`, `qwen…`) rather than guessing.
