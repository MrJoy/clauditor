...
`bundle exec bin/clauditor [--format table|csv|json] [--anthropic] [--summary] [--rollup] [--days N] [--since YYYY-MM-DD] [--verbose] [--project NAME] [--model NAME] [--utc] [--root DIR ...] [--no-store] [--store-dir DIR]`.
...
- **`Config`** loads optional defaults from a YAML file (`~/.clauditor_config`, injectable via `CLI.run(config_path:)` for tests). Every key (e.g. `project`, `model`, `format`) mirrors a CLI flag — except `remap`, which is config-only — and is translated to the same internal option symbols the CLI assembles, so `CLI#parse` layers them as **built-in defaults < config file < flags actually passed**. `remap` is a `{ project-path => project-path }` mapping (both sides `File.expand_path`-ed so `~` works); it folds stray project keys — typically long-gone worktrees the repo-root walk can no longer resolve — onto a canonical project, applied by the `Aggregator` after its automatic worktree reattachment. `--root` is special-cased: any `--root` on the CLI *replaces* the config roots wholesale (collected into a separate `cli_roots` so an absent flag leaves config intact). A missing file contributes nothing; an unknown key, ill-typed value, bad `format`, or malformed YAML raises `ArgumentError` (caught by the CLI and printed as `clauditor: …`).
...
- **`Aggregator`** is the heart. Claude Code writes one JSONL line per content block, and **every line sharing a `message.id` repeats the same message-level `usage`** — so it dedupes by `message.id` *globally* (resumed sessions replay earlier messages into new files). Summing raw lines overcounts several-fold. It groups by `(project, day, model)`, sums usage, and costs each cell. In `rows` it applies two remapping layers before the final merge: `ProjectNormalizer.build_remap` (loose worktree names → canonical checkouts) and then the config-supplied `remap:` hash (stray paths → user-chosen canonical), so a remapped project consolidates with the real checkout's cells. The remap also reaches seeded Store cells and is idempotent, so the consolidated result re-persists cleanly across runs.
...
- **`Pricing`** holds per-model USD/MTok rates plus cache multipliers (read 0.1×, 5m write 1.25×, 1h write 2×). A model's value is normally a flat `{ input:, output: }` hash, optionally with an explicit `cache_read:` USD/MTok rate when a model's cache reads don't follow the 0.1× rule (`fable-5-1` lists $0.25, `opus-5-5` $0.20); for models whose list price changes on a known date (e.g. `sonnet-5`'s introductory $2/$10 through 2026-08-31, then $3/$15) it's instead an array of tiers ordered oldest-first, each with an inclusive `until:` cutoff and the final tier open-ended. `cost_for`/`rates_for` take the cell's day and pick the active tier; a nil/`unknown` day falls back to the current open-ended tier. `normalize_model` reduces Claude ids to a concise name — dropping the `claude-` prefix and any trailing date stamp (`claude-haiku-4-5-20251001` → `haiku-4-5`) — which is also the id the report groups and displays by; non-Claude ids pass through untouched and cost `nil` (e.g. `<synthetic>`, `qwen…`) rather than guessing.
